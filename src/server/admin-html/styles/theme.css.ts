/**
 * Single Source of Truth for Dynamic Themes across Server-Side Rendered Pages
 * (Admin Dashboard, Public Share Landing, System Status).
 * Synchronizes with the React client app (src/index.css & useTheme.ts).
 */
...
export function getThemeStylesCss(): string {
  return THEME_CSS_VARIABLES;
}

export function getThemeLoaderScript(): string {
  return `${THEME_HEAD_SCRIPT}\n${THEME_BODY_SCRIPT}`;
}

export function getThemeSyncScript(): string {
  return THEME_STORAGE_LISTENER_SCRIPT;
}
